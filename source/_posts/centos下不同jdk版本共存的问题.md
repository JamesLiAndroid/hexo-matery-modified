---
title: centos下不同jdk版本共存的问题
top: false
cover: false
toc: true
mathjax: true
date: 2020-7-21 10:10:36
password:
summary:
tags:
categories:
...
ElasticSearch7.6的安装需要java11支持，而目前服务器安装的是java8（jdk1.8），而且es安装在另一个用户下，独自运行。
...
1. 切换到es用户，执行**./elasticsearch-7.6.2/bin/elasticsearch -d*命令，启动es

2. 切换到centos用户，执行**./apache-skywalking-apm-bin-es7/bin/statup.sh*，启动skywalking